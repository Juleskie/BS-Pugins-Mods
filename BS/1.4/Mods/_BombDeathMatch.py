...
def bsGetGames():
    return [BombDeathMatch]


class BombDeathMatch(bs.TeamGameActivity):

    #%%%
    @classmethod
    def getName(cls):
        return 'Bomb Deathmatch!'

    @classmethod
    def getDescription(cls, sessionType):
        return 'Throw bombs at your enemies and blow them up!'
    ...
    def getSettings(cls,sessionType):
        return [("Kills to Win Per Player",{'minValue':1,'default':5,'increment':1}),
                ...
                ("Bomb Type",{'choices':[('Normal Bombs',1),('Ice Bombs',2),('Impact Bombs',3),('Sticky Bombs',4),('Landmines',5)],'default':1}),
                ("Bomb Explosion Radius",{'minValue':0.1,'default':2.0,'increment':0.1}),
    ...
    def spawnPlayer(self, player):
        ...
        if self.settings["Bomb Type"]  == 3: bomb = 'impact'
        if self.settings["Bomb Type"]  == 4: bomb = 'sticky'
        if self.settings["Bomb Type"]  == 5: bomb = 'landMine'
        spaz.bombType = bomb
