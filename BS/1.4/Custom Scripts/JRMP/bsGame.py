...
class DiceMessage(object):
    ...
    Attributes:
      ...
      type
          Depending on the type, it might help the Dice Spaz punched with random effects.
          This variable specifies what kind we want.
         
    """
    def __init__(self,type='ironskin',special=False):
        """
        Instantiate with the given values.
        """
        self.type = type
...
class FireMessage(object):
    ...
    Attributes:
      ...
      sourceBomb
          Used to target the original player who lit the player.
          Used to gain kills if the victim is killed by fire DOT.
         
    """
    def __init__(self,time,source=None):
        """
        Instantiate with the given values.
        """
        self.time = time
...
class TeslaMessage(object):
    ...
    """
    def __init__(self,is_entered=True):
        """
        Instantiate with the given values.
        """
...
class DizzyMessage(object):
    ...
    Forces bots and players to put random inputs and generate noise.
    Generated by Dizzy Cake.
    """
    def __init__(self,source=None):
        """
        Instantiate with a given bs.Node.
...
class HitMessage(object):
    ...
    def __init__(self,srcNode=None,pos=Vector(0,0,0),velocity=Vector(0,0,0),
                 ...
                 hitSubType='default',hitSubTypeTrue=True):
        """
        ...
        self.hitSubTypeTrue=hitSubTypeTrue # The bomb type comes from the original source - is not passed to another bomb of another type
...
class Session(object):
    ...
    def onPlayerRequest(self,player):
        ...
        if bsUtils._gStressTestResetTimer is None:
            ...
            try: 
                if self._disableMaxPlayersText: return
            except:
                if len(self.players) >= self._maxPlayers:
                    ...
                    self._disableMaxPlayersText = True
                    return False
    ...
    def onPlayerLeave(self,player):
        ...
        if player in self.players:
            ...
            # remove this player's bomb shape choice from the list
            newList = []
            for sublist in bsUtils.gPlayerBombShapes:
                if sublist[0] == player: continue
                else: newList.append(sublist)
            bsUtils.gPlayerBombShapes = newList
            
            # same with catchphrases
            newList = []
            for sublist in bsUtils.gPlayerCatchphraseData:
                if sublist[0] == player: continue
                else: newList.append(sublist)
            bsUtils.gPlayerCatchphraseData = newList
    ...
    def _addChosenPlayer(self,chooser):
        ...
        player._setData(team=team,
                        ...
                        #bombShape=chooser.getBombShapeName())
           
        # keep track of each player's bomb shape choice
        # this list gets modified each time a player joins or leaves
        bsUtils.gPlayerBombShapes.append([player,chooser.getBombShapeName()])
        bsUtils.gPlayerCatchphraseData.append([player,chooser.getCatchphrases()])
...
class Activity(object):
    ...
    def __init__(self,settings={}):
        ...
        # if the activity includes a "crowd" - mostly for sports minigames
        # add additional sound effects when stuff happens during the game
        self._crowdActive = False
        
        # disable certain powerups on certain maps
        self._disableSpeed = False
        self._disableHijump = False
...
class JoiningActivity(Activity):
    ...
    def onTransitionIn(self):
        ...
        if isinstance(bs.getSession(),bs.FreeForAllSession):
            ...
            self._discordTypeImage = 'free_for_all'
            self._discordTypeText = 'Free For All'
        elif isinstance(bs.getSession(),bs.CoopSession):
            bs.playMusic('CharSelectCOOP')
            self._discordTypeImage = 'cooperative'
            self._discordTypeText = 'Cooperative'
        elif isinstance(bs.getSession(),bs.TeamsSession):
            bs.playMusic('CharSelectTEAMS')
            self._discordTypeImage = 'teams'
            self._discordTypeText = 'Teams'
        else: 
            bs.playMusic('CharSelectCOOP')
            self._discordType = 'Idle'
        ...
        import richPresence
        richPresence.message = 'Waiting for Players'
        richPresence.bigImage = 'lobby'
        richPresence.bigImageText = 'Lobby'
        richPresence.smallImage = self._discordTypeImage
        richPresence.smallImageText = self._discordTypeText
...
class GameActivity(Activity):
    ...
    def onBegin(self):
        ...
        if bs.getConfig().get('Party Mode', True) and not isinstance(bs.getSession(),bs.CoopSession): bs.gameTimer(10000,bs.Call(bsInteractive.diceSpawn))
        ...
        try:
            if isinstance(s,bs.CoopSession):
                ...
                self._presenceImage = 'cooperative'
                ...
                self._presenceImage = 'free_for_all'
        ...
        def _discordMapName():
            try: mapName = self.getMap().discordName
            except AttributeError: mapName = 'default'
            return mapName
        
        richPresence.message = 'Playing '+str(self._gamemode)
        richPresence.bigImage = _discordMapName()
        richPresence.bigImageText = self.getMap().getName()
        richPresence.smallImage = self._presenceImage
        richPresence.smallImageText = str(len(self.players))+" Player" if len(self.players) == 1 else str(len(self.players))+" Players"
    ...
    def setupStandardPowerupDrops(self,enablePowerups=True,enableTNT=True,enableHazards=True):
        """
        ...
        if enableHazards:
            self._platformObjs = {}
            self._bumperObjs = {}
            self._standardSpawnPlatform()
            self._standardSpawnBumper()
    ...
    def _standardSpawnPlatform(self,map=None):
        """
        Standard horizontal moving platform.
        """
        # spawn a moving platform at platform points if there are some
        for i,pt in enumerate(self.getMap().platformPoints):
            if not i in self._platformObjs: self._platformObjs[i] = {'absentTicks':9999,'obj':None}
            platformObj = self._platformObjs[i]
            platformObj['obj'] = bs.MovingPlatform(position=pt)
            
    def _standardSpawnBumper(self):
        """
        Standard bumper
        """
        # spawn a bumper at bumper points if there are some
        for i,pt in enumerate(self.getMap().bumperPoints):
            if not i in self._bumperObjs: self._bumperObjs[i] = {'absentTicks':9999,'obj':None}
            bumperObj = self._bumperObjs[i]
            bumperObj['obj'] = bs.Bumper(position=pt)
    ...
    def setupStandardTimeLimit(self,duration):
        ...
        self._standardTimeLimitText = bs.NodeActor(bs.newNode('text',
                                                              attrs={'vAttach':'top',
                                                                     ...
                                                                     'position':(-25,-45) if self.getName() == 'Elusive Challenge' else (-25,-30),
                                                                     'flatness':1.0,
                                                                     'scale':1.2 if self.getName() == 'Elusive Challenge' else 0.9}))
    ...
    def _standardTimeLimitTick(self):
        ...
        if self._standardTimeLimitTime <= 10:
            if self._standardTimeLimitTime == 10:
                ...
                if self.getName() == 'Elusive Challenge': 
                    self._standardTimeLimitText.node.position = (-20,-50)
                    self._standardTimeLimitText.node.scale = 1.6
                else: 
        ...
        if self._standardTimeLimitTime <= 0:
            ...
            n = bs.newNode('text',
                           attrs={'vAttach':'top','hAttach':'center',
                                  ...
                                  'position':(0,-100) if self.getName() == 'Elusive Challenge' else (0,-90),'scale':1.2,
    ...
    def showZoomMessage(self,message,color=(0.3,0.7,0.9),scale=0.8,duration=2000,trail=False):
        """
