...
class snoBall(bs.Actor):
    ...
    def __init__(self, position=(0,1,0), velocity=(5,0,5), sourcePlayer=None, owner=None, explode=False):
        ...
        if factory._ballsMelt: #defaults to True.
            ...
            bs.gameTimer(3000, bs.WeakCall(self._disappear))
        self._hitNodes = set()
                ...
                if bs.Vector(*v).length() > 5.0:
                    ...
                    if node is not None and node.exists() and not node in self._hitNodes:
                        ...
                        node.handleMessage(bs.HitMessage(pos=t,
                                                         velocity=v,
                                                         magnitude=0.9, #bsVector.Vector(*v).length()*0.5,
                                                         velocityMagnitude=0.9, #0.5, #bsVector.Vector(*v).length()*0.5,
                                                         radius=0,
                                                         srcNode=self.node,
    ...
    def doBust(self):
        if self.exists():
            if not self._exploded:
                self._exploded = True
                bs.emitBGDynamics(position=self.node.position,velocity=[v*0.1 for v in self.node.velocity],count=10,spread=0.1,scale=0.4,chunkType='metal')
                ...
                bs.gameTimer(50,bs.WeakCall(self.handleMessage,bs.DieMessage()))
    ...
    def calcBust(self, oVel):
        ...
        if oSpd*oSpd - dot > 50.0:
            ...
            bs.gameTimer(25, bs.WeakCall(self.doBust))
...
class SnoBallFactory(object):
    ...
    def __init__(self):
        ...
        self.texSno = bs.getTexture("bombColor")
        ...
        self.defaultBallTimeout = 300 #300
    ...
    def throwBall(self, spaz):
        ...
        if t - spaz.lastBallTime > bTime:
            ...
            def noball():
                ...
                direction[1] = 0.05 #This is the upward throw angle
            ...
            bs.gameTimer(100,noball)
            bs.gameTimer(150,noball)
            bs.gameTimer(200,noball)
