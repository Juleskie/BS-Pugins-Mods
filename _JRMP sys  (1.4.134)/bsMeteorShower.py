...
class MeteorShowerGame(bs.TeamGameActivity):
    ...
    def getSettings(cls, sessionType):
        return [("Epic Mode", {'default': False}), ("Disable Bomb's Velocity", {'default': True}),
("Bomb Type",{'choices':[('Normal',0),('Frozen',1),
                                        ('Combat',2),('Grenade',3),
                                        ('Ranger',4),('Knocker',5),
                                        ('Dynamite',6),('Sticky',7),('Snowball',8),('Random Easy',9),
                                        ('Random Hard',10)],'default':0})]
    ...
    def onBegin(self):
        ...
        self._timer.start()
        if self.settings["Bomb Type"]  == 0: 
            self.bomb = 'normal'
            self.dropRate = 0.2
        elif self.settings["Bomb Type"]  == 1:
            self.bomb = 'ice'
            self.dropRate = 0.2
        elif self.settings["Bomb Type"]  == 2:
            self.bomb = 'combat'
            self.dropRate = 0.1
        elif self.settings["Bomb Type"]  == 3:
            self.bomb = 'grenade'
            self.dropRate=0.25
        elif self.settings["Bomb Type"]  == 4:
            self.bomb = 'ranger'
            self.dropRate = 0.4
        elif self.settings["Bomb Type"]  == 5:
            self.bomb = 'knocker'
            self.dropRate = 0.2
        elif self.settings["Bomb Type"]  == 6:
            self.bomb = 'dynamite'
            self.dropRate = 0.3
        elif self.settings["Bomb Type"]  == 7:
            self.bomb = 'sticky'
            self.dropRate = 0.22
        elif self.settings["Bomb Type"]  == 8:
            self.bomb = 'snowball'
            self.dropRate = 0.15
        elif self.settings["Bomb Type"]  == 9:
            self.bomb = 'normal'
            self.dropRate = 0.2
        elif self.settings["Bomb Type"]  == 10:
            self.bomb = 'normal'
            self.dropRate = 0.2
    ...
    def _setMeteorTimer(self):
        bs.gameTimer(int((1+self.dropRate*random.random())*self._meteorTime),self._dropBombCluster)
    ...
    def _dropBombCluster(self):
        ...
        for i in range(random.randrange(1, 3)):
            ...
            pos = (-7.3+15.3*random.random(), 11, -5.5+3.1*random.random())
            if self.settings['Disable Bomb\'s Velocity'] == True:
    ...
    def _dropBomb(self, position, velocity):
    	if self.settings['Bomb Type'] == 9: self.bomb = random.choice(['sticky','normal','normal','normal','ice','combat','combat','grenade','grenade','knocker'])
        if self.settings['Bomb Type'] == 10: self.bomb = random.choice(['snowball','sticky','normal','normal','ice','combat','combat','grenade','ranger','knocker','knocker','dynamite'])
        b = bs.Bomb(position=position, velocity=velocity, bombType=self.bomb).autoRetain()
