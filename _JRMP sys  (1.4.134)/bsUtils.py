...
_gPlayerColors = [
                  (0.5,0,0.7),  (0.1,0.1,1),  (0.5,0.25,1.0),  (0.5,0.3,0.45),  (1,0.15,0.15),  (0.1,0.5,0.7),  (0.3,0.3,0.3), (0.13,0.13,0.13),
                  (0.2,1,0.2),  (0.75,0.7,0.35),  (0.5,0.5,0),  (0.1,0.1,0.5),  (0.8,0,1.0),  (0.5,0.5,1),  (0.5,0.5,0.5), (1,0.3,0.5),
                  (0.4,0.5,0.4),  (0,0.85,0.7),  (0.2,1,1),  (0.6,0.25,0.25),  (1.3,0.8,1.0),  (0.05,0.65,0.05),  (1,0.5,0), (0.4,0.05,0.05),
                  (0,0,0),  (1,0.8,0.5),  (1,1,0),  (1,0.9,0.8),  (0,0.25,0.25),  (1.2,0.35,0),  (0.4,0.2,0.1), (1,1,1),
                  (0.9,0.55,0.3),  (0.1,0.35,0.1),  (1,1,0.5),  (1,0.5,0.5),  (1,0.8,0),  (0.8,1,0),  (0.6,0.9,1), (0.8,0,0.5)
]
                  
_gPlayerCatchphrases = ['Check this out!','Gotcha!','Fear me, cowards!',
                        'That was epic!','I\'m a beast!','Loser!',
                        'Don\'t even try!','Fight me, I dare you.',
                        'Did you see that?','Look at me!','Fire in the hole!','Noob!',
                        'Best of the bunch!','Combat Time!','Eat this!','Not bad',
                        'Are you scared?','You won\'t get me!','Im da best','Yum',
                        'Take that!', 'I did it!','Ha-Ha!']
                  
gPlayerBombShapes = []

gPlayerCatchphraseData = []
...
def checkTime(useInternet=True): # JRMP
...
def statAdd(statName): # JRMP
    """
    category: General Utility Functions

    Used by Joyride Modpack to save player and game
    statistics in the config file.
    """
    if not bs.getConfig().get('Cheats Active',True):
    ...
    if activity is not None:
        ...
        if name == 'globals':
            obj = bs.newNode('globals')
                      ...
                      'footingMaterial','attackMaterial','bossMaterial','powerupMaterial',
                      'fireMaterial','iceMaterial','burgerMaterial']:
            obj = bs.Material()
            ...
            obj = bs.Material()
        elif name == 'platformMaterial':
            obj = bs.Material()
            #obj.addActions(conditions=(('theyHaveMaterial',bs.getSharedObject('footingMaterial'))),
...
def _getDefaultCoopPlaylist(): # Coop Default Playlist
    import bs.Additional
    return [
    {
        "type": "bsAdditional.NightmareGame"
    }
   ]
...
def _getDefaultFreeForAllPlaylist(): # JRMP FFA Default Playlist
    ...
    }, 
    {
        ...
        "type": "bsArmsRace.ArmsRaceJRMPGame"
    }, 
    {
        "settings": {
        "Allow Negative Scores": False, 
    ...
    }, 
    {
        "settings": {
        "Epic Mode": False, 
        "Points to Win Per Player": 5, 
        "Respawn Times": 1.0, 
        "Time Limit": 300, 
        "Time to Kill": 8, 
        "map": "Crag Castle"
        }, 
        "type": "bsGetTheTarget.GetTheTargetGame"
    }, 
    {
        "settings": {
        "Allow Negative Scores": False, 
    ...
    {
        "settings": {
        "Elimination Timer": 20, 
        "Epic Mode": False, 
        "map": "Crag Castle"
        }, 
        "type": "bsHotPotato.HotPotatoGame"
    }, 
    {
        "settings": {
        "Allow Negative Scores": False, 
    ...
    }, 
    {
        ...
        "Respawn Times": 0.5, 
        "Time Limit": 300, 
        "map": "Hovering Plank-o-Wood"
        }, 
        "type": "bsDeathMatch.DeathMatchGame"
    }, 
    {
        "settings": {
        "Elimination Timer": 15, 
        "Epic Mode": False, 
...
def getGameTypes():
    ...
    for module in modules:
    	#if ((module.__name__ != 'bsTargetPractice' or bsInternal._getPurchased('games.target_practice'))
...
def _getAllTips():
    ...
    tips += [
            ...
            'Repedately punching will decrease your punch power.\nInstead go for rarer, but accurate punches for the most damage.',
            ...
            'Sticky Bombs can block passages for a short amount of time.\nYou can use this while you\'re chased down.',
            ...
            'Healing Bombs can thaw your teammates, fix damaged\nEnergy Shields and remove bombs without detonating them.',
            'Don\'t be afraid to throw it at your enemies!\nThat\'ll damage them, instead of healing.',
            'Healing Bombs are great for helping your teammates during a fight.\nThrow it at them to give \'em a boost!',
            #Grenade tips
            ...
            'Grenades are stronger than regular bombs.\nDon\'t waste that power!',
            'You can use one Grenade to detonate the other.\nUse this for quick kills!',
            #JRMP Menu tips
            'On the Main Menu, there\'s a JRMP button, where\nyou can adjust specific features of the modpack.',
            ...
            'Bombs buffed by Bigger Blasts powerup are SCARY!\nAvoid them at all cost! (even then they might hit you)',
            #Land Mines
            'Land Mines placed even in the most random places\nyou can imagine can make some serious threat.',
            'Land Mines take two explosions to destroy.',
            #Speed Boots tips
            'Speed Boots replaces Boxing Gloves.\nTake the powerup that will be the most valuable at that moment.',
            'Speed Boots make your punches faster, but a little bit weaker.',
            'When using Speed Boots, be careful on ramps.\nYou might launch yourself from it.',
            #Life Quality tips
            'To use all the powerups,enable Use Hard Powerups.\nOnly do this if you\'re accustomed to other powerups.',
            #Dizzy Cake tips
            'Dizzy Cake takes time to explode.\nUse it for area denial rather than killing.',
            'Dizzy Cake cannot be picked up, so don\'t worry about throwing it near the enemy.',
            'Bombs jump towards the nearest target when under the\ninfluence of Dizzy Cake! Watch out!',
            'When Dizzy Cake is about to explode, time your tackle just \nbefore an explosion. If done correctly, you won\t get dizzy',
            #Magnet Marble
            'Be careful while using the Magnet Marble.\nIt chases its nearest target - even you!',
            'Magnet Marble becomes more agressive over time!\nMake sure it locks on to someone else or falls out of bounds.',
            #Fire Bomb
            'Fire Bomb deals 3/4 of player\'s health via fire.\nBe sure not to get caught!',
            'While lit by a Fire Bomb, your punches are stronger\nand you can lit people!. Go spread it like wildfire!',
            'If someone\'s on fire, Fire Bomb explosions deal more damage.\nThrow another one to hopefully finish your target off!',
            'Fire melts Energy Shields real good.',
            #Hi-Jump
            'Running while Hi-Jumping propels you forward rather than upward.\nUse it to better control your flying and landing.',
            'Hi-Jump explosion deals massive damage to everyone\nwho stands right below you!',
            'You don\'t get stunned for your first Hi-Jump off the ground.\nUse it to Hi-Jump again mid-air or punch someone when you land!',
            #Throw Teching
            'If someone tries to pick you up, immediately press the Pick Up button.\nIf you do it right, you can break free and gain an upper hand.',
            'Spamming Pick-Up will get you nowhere.\nYou deal no damage and you can be easily countered.',
            'If you know your enemy is going to grab you, press the Pick Up button\nshortly after he grabs you. You\'ll be rewarded!',
            #Dash Tackle
            'Press jump and punch at the same time to do a tackle.\nUse it to evade explosions and deal damage to enemies.',
            'Using a tackle leaves you vulnerable for a short time.\nUse it wisely!',
            'Tackle is a great finishing move due to its high damage, but don\'t overuse\nthat ability. The stun on a successful tackle is huge.',
            'Tackle can be a great maneuver - if you aim it well.\nMissing can make you jump off a cliff, which is lovely.',
            'Tackle at a precise moment to avoid getting frozen, lit on fire or messed up by the Dizzy Cake.',
            # Bomb Shapes
            'Bomb shapes are cosmetic only.\nThey roll exactly the same way as regular bombs.',
            ]
   ...
   # IMPROVED MOD TIPS!
    ...
    if bs.getEnvironment()['platform'] in ('android','ios'):
        tips += [('If your device gets too warm or you\'d like to conserve battery power,\n'
                 ...
                 ('Tackling can be difficult when using a touch screen.\n'
                  'Press jump then quickly slide onto the punch button to perform it.'),
                 ('Playing JRMP with a touch screen can be difficult.\n'
                  'Consider plugging in a gamepad.')]
...
def _setLanguage(language,printChange=True,storeToConfig=True):
    ...
    try:
        ...
        l = __import__('bsLanguageEnglish')
    ...
    with bs.Context('UI'):
        ...
        if musicType is not None:
            ...
            if musicType == 'Menu':
                ...
                volume = 6.0
                loop = False
            elif musicType == 'Coop Scores':
                filename = 'coopScoresMusic'
                volume = 2.0
                ...
                filename = 'runAwayMusic'
                volume = 6.5
            elif musicType == 'JRMP Onslaught':
                filename = 'jrmpOnslaughtMusic'
                volume = 8.0
            elif musicType == 'JRMP Runaround 1':
                filename = 'jrmpRunaroundMusic1'
                volume = 8.0
            elif musicType == 'JRMP Runaround 2':
                filename = 'jrmpRunaroundMusic2'
                volume = 8.0
            elif musicType == 'JRMP Runaround 3':
                filename = 'jrmpRunaroundMusic3'
                volume = 8.0
            ...
            # MARATHON
                ...
                loop = False
            #SCRAPPED
            elif musicType == 'Spy Brothers':
                filename = 'spyBrothersMusic'
                volume = 10.0
                loop = True
            elif musicType == 'Cardboard':
                filename = 'cardboardMusic'
                volume = 10.0
                loop = True
            # OTHER
            elif musicType == 'Menu Night':
                filename = 'menuNightMusic'
                volume = 5.0
            else:
...
def getPlayerProfileBombShape(profileName):
    """ given a profile, returns the customized bomb shape string for it """
    
    bsConfig = bs.getConfig()
    try: bombShape = bsConfig['Player Profiles'][profileName]['bombShape']
    except Exception: bombShape = 'normal'
    return bombShape
    
def getPlayerProfileCatchphrases(profileName):
    """ given a profile, returns the customized catchphrase strings for it """
    
    bsConfig = bs.getConfig()
    try: catchphrases = bsConfig['Player Profiles'][profileName]['catchphrases']
    except Exception: 
        catchphrases = []
        for i in range(4): catchphrases.append(random.choice(_gPlayerCatchphrases))
    return catchphrases
...
def _startStressTest(args):
    ...
    elif playlistType == 'Coop':
    	bsConfig['Team Tournament Playlist Selection'] = args['playlistName']
        bsConfig['Team Tournament Playlist Randomize'] = 1
        bs.realTimer(1000,bs.Call(bs.pushCall,bs.Call(bsInternal._newHostSession,bs.CoopSession)))
    else:
...
class ControlsHelpOverlay(bsGame.Actor):
    ...
    def __init__(self,position=(390,120),scale=1.0,delay=0,lifespan=None,bright=False):
        ...
        #self._tackleTextPosTop = (position[0],position[1] - 160.0*scale)
        #self._tackleTextPosBottom = (position[0],position[1] - 197.0*scale)
        #self._tackleText = bs.newNode('text',
        #                        attrs={'scale':1.0*scale if bs.getEnvironment()['vrMode'] else 0.8*scale,
        #                               'hostOnly':True,
        #                               'shadow':1.0 if bs.getEnvironment()['vrMode'] else 0.5,
        #                               'flatness':1.0,
        ...
        self._nodes = [self._bombImage,self._bombText,
                       ...
                       self._runText,#self._tackleText,
                       self._extraText]
    ...
    def _update(self):
        ...
        if all(len(l) == 0 for l in (punchButtonNames,jumpButtonNames,bombButtonNames,pickUpButtonNames)):
            ...
            # otherwise show standard buttons
            elif bs.getEnvironment()['subplatform'] == 'oculus':
        ...
        #self._tackleText.text = tackleText
...
class Background(bsGame.Actor):
    ...
    def __init__(self,fadeTime=500,startFaded=False,showLogo=False):
        ...
        with bs.Context(session):
            ...
            if showLogo:
                logoTexture = bs.getTexture('logoJRMP')
                ...
                # Logo beeps red
                self.r = bs.newNode('combine',owner=self.node,attrs={'input1':self.logo.color[1],'input2':self.logo.color[2],'size':3})
                animate(self.r,'input0',{0:self.logo.color[1], 50:0.4, 500:self.logo.color[1]},loop=True)
                self.r.connectAttr('output',self.logo,'color')
...
class PopupText(bsGame.Actor):
    ...
    """
    def __init__(self,text,position=(0,0,0),color=(1,1,1,1),randomOffset=0.5,offset=(0,0,0),scale=1.0,lifespan=1500):
        """
...
class ZoomText(bsGame.Actor):
    ...
    def __init__(self,text,position=(0,0),shiftPosition=None,shiftDelay=None,lifespan=None,flash=True,trail=True,hAlign="center",
                 color=(0.3,0.7,0.9),jitter=0.0,trailColor=(0.1,0.35,1.0,0.0),scale=1.0,projectScale=1.0,tiltTranslate=0.0,maxWidth=None):
        bs.Actor.__init__(self)
        self._dying = False
...
def _configServer():
    ...
    name = "[JRMP] " + config.get('partyName', 'party')
...
def _runServer():
    ...
    if _gRunServerFirstRun:
        ...
        print 'BombSquad Joyride Modpack 2.7 is running!'
    ...
    antiAFK = config.get('antiAFK', 0)
    powerupDistribution = config.get('powerupDistribution', 'JRMP')
    kickIdle = config.get('kickIdlePlayers', False)
    partyMode = config.get('partyMode', False)
    ...
    bs.getConfig()['Kick Idle Players'] = kickIdle
    bs.getConfig()['Powerup Distribution'] = powerupDistribution
    bs.getConfig()['Party Mode'] = partyMode
    bs.getConfig()['AFK Time'] = antiAFK
    bs.getConfig()['Port'] = config.get('port',43210)
    bs.writeConfig()
    
    # set series lengths
    bsTeamGame.gTeamSeriesLength = config.get('teamsSeriesLength', 5)
