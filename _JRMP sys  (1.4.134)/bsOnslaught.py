...
class OnslaughtGame(bs.CoopGameActivity):
    ...
    def __init__(self,settings={}):
        ...
        if self._preset in ['training','trainingEasy','pro','proEasy','expert','expertEasy','onsExpert','onsExpertEasy']: settings['map'] = 'Doom Shroom Large'
        elif self._preset in ['jrmpEndless']: settings['map'] = 'JRMP Onslaught'
        elif self._preset in ['endless']: settings['map'] = 'Shiny Shroom'
        ...
        elif settings['map'] == 'Shiny Shroom':
            self._spawnCenter = (0,3,-5)
            self._tntSpawnPosition = (0,3,-5)
            self._powerupCenter = (0,5,-3.6)
            self._powerupSpread = (6,4)
        elif settings['map'] == 'JRMP Onslaught':
    ...
    def onTransitionIn(self):
        ...
        self._spawnInfoText = bs.NodeActor(bs.newNode("text",
                                                      attrs={'position':(15,-130),
                                                             ...
                                                             'text':''}))
        if self._preset in ['jrmpEndless']: bs.playMusic('JRMP Onslaught')
        else: bs.playMusic('Onslaught')
    ...
    def onBegin(self):
        ...
        if self._preset  in ['training','trainingEasy']:
            ...
            self._haveTnt = False
            self._excludePowerups = ['curse','landMines','hijump','clusterBomb','knockerBombs','bunny']
            self._waves = [

                {'baseAngle':195,
                 'entries':([
                            {'type':bs.BomberBotLame,'spacing':50},
                            ] * playerCount)},
                ...
                {'baseAngle':195,
                 'entries':([
                            {'type':bs.ToughGuyBotLame,'spacing':5},
                            {'type':bs.BomberBotLame,'spacing':25},
                            {'type':bs.ChickBotLame,'spacing':90},
                            {'type':bs.ToughGuyBotLame,'spacing':5} if playerCount > 1 else None,
                            {'type':bs.ChickBotLame,'spacing':90} if playerCount > 2 else None
                            ])},
                ]

        elif self._preset in ['rookie','rookieEasy']:
            self._haveTnt = False
            self._excludePowerups = ['curse','knockerBombs','hijump','clusterBomb','bunny']
            self._waves = [

                {'entries':[
                        {'type':bs.NinjaBotLame,'point':'LeftUpperMore'} if playerCount > 2 else None,
                        {'type':bs.NinjaBotLame,'point':'LeftUpper'},
                        ]},
                ...
                {'entries':[
                        ...
                        {'type':bs.ChickBotLame,'point':'Left'},
                        {'type':bs.ChickBotLame,'point':'LeftLower'} if playerCount > 1 else None,
                        {'type':bs.ChickBotLame,'point':'LeftUpper'} if playerCount > 2 else None,
                        ]},
                {'entries':[
                ...
                {'entries':[
                        {'type':bs.ChickBotStaticLame,'point':'TurretBottomLeft'},
                        {'type':bs.ChickBotStaticLame,'point':'TurretBottomRight'},
                        {'type':bs.ChickBotLame,'point':'Bottom'},
                        {'type':bs.ChickBotLame,'point':'BottomHalfRight'} if playerCount > 1 else None,
                        {'type':bs.ChickBotLame,'point':'BottomHalfLeft'} if playerCount > 2 else None,
                        ]},

                {'entries':[
                        {'type':bs.BomberBotStaticLame,'point':'TurretTopLeft'},
                        {'type':bs.BomberBotStaticLame,'point':'TurretTopRight'},
                        {'type':bs.NinjaBotLame,'point':'Bottom'},
                        {'type':bs.NinjaBotLame,'point':'BottomHalfLeft'} if playerCount > 1 else None,
                        {'type':bs.NinjaBotLame,'point':'BottomHalfRight'} if playerCount > 2 else None,
                        ]},
                ]

        elif self._preset in ['pro','proEasy']:
            self._excludePowerups = ['curse','hijump','bunny']
            self._haveTnt = True
            self._waves = [
                ...
                {'baseAngle':90,
                 'entries':([
                            {'type':bs.MelBotLame,'spacing':50},
                            {'type':bs.MelBotLame,'spacing':50} if self._preset == 'pro' else None,
                            {'type':bs.MelBotLame,'spacing':50},
                            {'type':bs.MelBotLame,'spacing':50} if playerCount > 1 else None,
                            {'type':bs.MelBotLame,'spacing':50} if playerCount > 3 else None,
                            ])},
            ...
            self._haveTnt = True
            self._excludePowerups = ['knockerBombs','hijump','clusterBomb','bunny']
            self._waves = [
        ...
        elif self._preset in ['endless','endlessTournament','jrmpEndless']:
            self._haveTnt = True
            self._excludePowerups = ['hijump']
    ...
    def spawnPlayer(self,player):
        ...
        if self._preset in ['trainingEasy','rookieEasy','proEasy','uberEasy','expertEasy','ace']:
            s._impactScale = 0.25
    ...
    def _updateWaves(self):
        ...
        if self._canEndWave and not self._bots.haveLivingBots() and not self._gameOver:
            ...
            if self._preset in ['endless','endlessTournament','jrmpEndless']: won = False
    ...
    def _updatePlayerSpawnInfo(self):
        ...
        if not any(player.isAlive() for player in self.teams[0].players):
            ...
            t = ''
            for player in self.players:
                if not player.isAlive() and (self._preset in ['endless','endlessTournament','jrmpEndless'] or (player.gameData['respawnWave'] <= len(self._waves))):
    ...
    def _startNextWave(self):
        ...
        elif self._preset in ['jrmpEndless']: #JRMP ENDLESS BOT TYPES
        ...
        for info in entries:
            ...
            if point is not None:
                ...
                if botType is not None:
                    ...
                    t += dt
                if botType is [bs.PirateBot,bs.PirateBotRadius]: botAngle += spacing*0.25 #Suiciders aren't close enough to blow themselves up so easily, leading to free points
                if self._preset in ['jrmpEndless']: botAngle += spacing*0.75
                else: botAngle += spacing*0.5
        ...
        self._waveText = bs.NodeActor(bs.newNode('text',
                                                 attrs={'vAttach':'top',
                                                        ...
                                                        'text':bs.Lstr(value='${A} ${B}',subs=[('${A}',bs.Lstr(resource='waveText')),('${B}',str(self._wave)+('' if self._preset in ['endless','endlessTournament','jrmpEndless'] else ('/'+str(len(self._waves)))))])}))
    ...
    def addBotAtAngle(self,angle,spazType,spawnTime=1000):
        ...
        if self._preset in ['jrmpEndless']: x = math.sin(angleRadians)*0.9
        else: x = math.sin(angleRadians)*1.06
        if self._preset in ['jrmpEndless']: z = math.cos(angleRadians)*1.15
        else: z = math.cos(angleRadians)*1.06
